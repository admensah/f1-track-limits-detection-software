...
class SingleShapeDrawingApp:
    ...
    def lines(filename):
        ...
        lines = cv2.HoughLinesP(
            ...
            threshold=150,  # Increase min number of votes for valid line
            minLineLength=25,  # Increase min allowed length of line
            maxLineGap=10  # Decrease max allowed gap between line for joining them
        )
        ...
        # Write the result
        cv2.imwrite(f'output_with_lines{filename}', background_image)

def main():
    for i in {1,2,3 ,6,7}:
        filename = "edges_" + str(i) + ".jpg"
        root = tk.Tk()
        app = SingleShapeDrawingApp(root, filename)
        root.mainloop()
        SingleShapeDrawingApp.lines(str(i) + ".jpg")
